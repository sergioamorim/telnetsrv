...
# --------------------------- Environment Setup ----------------------------
    ...
    def setterm(self, term):
        ...
        curses.setupterm(term, fd=1) # This will raise if the termtype is not supported
        self.TERM = term
        self.ESCSEQ = {}
        for k in self.KEYS.keys():
            ...
            if str:
                self.ESCSEQ[str] = k
        self.CODES['DEOL'] = curses.tigetstr('el') or b'\x1b[K'.decode('ascii')
        self.CODES['DEL'] = curses.tigetstr('dch1') or b'\x1b[P'.decode('ascii')
        self.CODES['INS'] = curses.tigetstr('ich1') or b'\x1b[@'.decode('ascii')
        self.CODES['CSRRIGHT'] = curses.tigetstr('cuf1') or b'\x1b[C'.decode('ascii')
        self.CODES['CSRLEFT'] = curses.tigetstr('cub1') or b'\x08'.decode('ascii')
    ...
    def setup(self):
        ...
        for k in self.DOACK.keys():
            continue  # self.sendcommand(self.DOACK[k], k)
        for k in self.WILLACK.keys():
            continue  # self.sendcommand(self.WILLACK[k], k)
...
# ------------------------- Telnet Options Engine --------------------------
    ...
    def sendcommand(self, cmd, opt=None):
        ...
        if cmd in [DO, DONT]:
            if not opt in self.DOOPTS.keys():
            ...
            if not opt in self.WILLOPTS.keys():
                self.WILLOPTS[opt] = ''
        ...
        while True:
            ...
            if c == theNULL:
                ...
                if insptr > 0:
                    ...
                    self._readline_echo(BELL, echo)
                    self._readline_echo(self.PROMPT[-1:], echo)
                continue
                ...
                if len(line) > insptr:
                    ...
                    pass  # self._readline_echo(c, echo)
...
# --------------------------- Output Functions -----------------------------
    ...
    def writecooked(self, text):
        ...
        text = ''.join(filter(lambda x: ord(x) < 128, text)).encode('ascii')
...
# ------------------------------- Input Cooker -----------------------------
    def _inputcooker_getc(self, block=True):
        ...
        try:
            ...
            ret = None
        self.eof = not(ret)
        self.rawq += ret.decode('ascii') if ret else ''
        if self.eof:
            raise EOFError
    ...
    def cmdHELP(self, params):
        ...
        for cmd in keys:
            ...
            if getattr(method, 'hidden', False):
                continue
            doc = method.__doc__.split("\n") if method.__doc__ else ['', 'no description available']
            docp = doc[0].strip()
            docs = doc[1].strip()
            if len(docp) > 0:
                docps = "%s - %s" % (docp, docs, )
    ...
    def authentication_ok(self):
        ...
        if self.authCallback:
            if self.authNeedUser:
                username = self.readline(prompt="Login: ", use_history=False)
        ...
        while self.RUNSHELL:
            ...
            if self.input.cmd:
                ...
                if cmd in self.COMMANDS.keys():
                    ...
                    if self.PROMPT == 'Password: ':
                        self.writeerror(text='incorrect password\n')
                        self.PROMPT = 'User> '
                    else:
                        self.writeerror("Unknown command '%s'\n" % cmd)
